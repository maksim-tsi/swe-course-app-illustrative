...
Use `dev-max` as the instructor demonstration integration branch; `main` remains the stable branch with its active `Protect Main` ruleset. Before any edit, verify the repository and current status, fetch `origin/dev-max`, and start a descriptive `feature/<task>` branch from that ref in a **new isolated Git worktree**. Every agent session, including reviewers, works in its own worktree. Do not let multiple agents edit one worktree or use the primary checkout as a scratchpad. Preserve unrelated changes and history; never reset or force-push to make a branch fit.

When an agent changes files for instructor practice or demonstration, commit only the bounded task, push its feature branch, and open a PR with base `dev-max`. Do not push directly to `dev-max` or `main`, or open these feature PRs against `main`. A read-only reviewer reports findings on the existing PR and does not create an empty PR. Leave the PR open for instructor review; an AI reviewer is advisory and may not merge or treat its own approval as human acceptance. If another agent needs the change, hand off the commit/branch/PR rather than uncommitted files in a shared tree. Rebase or merge an updated `dev-max` only after inspecting conflicts and without discarding work. Changing the GitHub default branch or promoting `dev-max` to `main` requires a separate instructor decision.
